...
6. READ ADDRESS 0000-0001
    データとバッファの状態を確認する
    データは00-3F(EOS)
    ダミーリード(20,20)

7. READ ADDRESS 0000-0001 (32B)
    READシーケンスを途中で止めて、バッファの中身を20,21にする

8. READ ADDRESS 0000-0000 (0B) (REC=1)
    REC=1にするがデータアクセスはしない

9. READ ADDRESS 0000-0001
    データとバッファの状態を確認する
    データは00-3F(EOS)
    ダミーリード(20,21)

全体
    REC=1でREADしてもメモリの内容は変わらない
    バッファの先頭データが返されるが、2バイト読んだところでBRDYは0になり、バッファが空になる
    その後のREADではバッファの2バイト目も1バイト目と同じ値になる
    REC=1にしただけではバッファの中身は変わらず、READした時に変わる様子
...
rs.reset()
rs.seq_mem_read(0x0000, 0x0001, 64+2, "6. READ ADDRESS 0000-0001")
rs.reset()
rs.seq_mem_read(0x0000, 0x0001, 32+2, "7. READ ADDRESS 0000-0001 (32B)")
rs.reset()
test_seq_mem_read_with_record(rs, 0x0000, 0x0000, 0, "8. READ ADDRESS 0000-0000 (0B) (REC=1)")
rs.reset()
rs.seq_mem_read(0x0000, 0x0001, 64+2, "9. READ ADDRESS 0000-0001")
