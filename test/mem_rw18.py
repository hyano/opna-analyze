...
def test_seq_mem_read_with_record(self, start, stop, count, repeat, msg):
    if repeat:
        control1 = 0x70
    else:
        control1 = 0x60
...
'''
REC=1でREADした時の挙動の確認 (REPEATを変えて、過剰に読み出し)
...
5. READ ADDRESS 0000-0000 (REC=1/REPEAT=0)
    REC=1/REPEAT=0でREADする(68バイト)
...
rs.reset()
test_seq_mem_read_with_record(rs, 0x0000, 0x0000, 64+4, False, "5. READ ADDRESS 0000-0000 (REC=1/REPEAT=0)")
rs.reset()
rs.seq_mem_read(0x0000, 0x0001, 32+2, "6. READ ADDRESS 0000-0001 (32B)")
rs.reset()
test_seq_mem_read_with_record(rs, 0x0000, 0x0000, 64+4, True, "7. READ ADDRESS 0000-0000 (REC=1/REPEAT=1)")
rs.reset()
rs.seq_mem_read(0x0000, 0x0001, 64+2, "8. READ ADDRESS 0000-0001")
