...
'''
READシーケンスを途中で止めた場合のダミーリードの確認 (REPEAT=1)
...
全体
    ...
    34B目以降の動きがREPEAT=0の時よりも1バイト早い(1Fが一回少ない)
...
for i in [1, 2, 3, 4, 31, 32, 33, 34, 35, 36, 37]:
    rs.reset()
    test_seq_mem_read(rs, 0x0000, 0x0000, i, "{0}. READ ADDRESS 0000-0000 ({1}B) (REPEAT=1)".format(testno, i))
    testno += 1
    rs.reset()
