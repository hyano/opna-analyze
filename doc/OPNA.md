...
* 再生開始直後に2バイトのデータ(D0D1,D2D3)がメモリから読み出される。
...
* D60再生後の出力レベルが維持される。
* 再生中にREG00に$A1を書いたとき(RESETビット)に再生が停止し、出力レベルが変化しなくなる。
  * ステータスレジスタのEOSビットが0だった場合も、1になる。
    (再生中の場合EOSが1になる様子。)
* 出力レベルは、REG00に$00を書いたときに0になる。ただし、次の再生時に見えるので内部では維持されている模様。
  * この時にステータスレジスタのPCMBUSYビットも0になる。
  * ステータスレジスタのEOSビットが0だった場合も、1になる。
  * ステータスレジスタのBRDYビットが0だった場合も、1になる。
    * 再生開始後にクリア(REG10に0x80を書き込み)した場合、ここまで1にならない。
...
* 32バイト(64ニブル D0-D63)のデータのうち、D60までが再生され、最後の3ニブルのデータは再生されない。
* D60の再生が終わったところで、ステータスレジスタのEOSビットが1になる。
  * 再生中にクリアした場合も、再度D60再生が終わったタイミングで1になる。
* 同時に2バイトのデータ(D0D1,D2D3)がメモリから読み出される。
...
### REPEATビットを1にして読み出した時の動作

REPEATビットを1にしてデータの読み出した時の挙動。
...
![REPEATビットを1にして読み出した時の動作](../chart/mem_read_repeat.png)

* 32バイト目のデータを読み出したところで、ステータスレジスタのEOSビットが1になる。
* 33バイト目の読み出しでSTART ADDRESSに戻る。
* その後の1バイトはダミーリードとなる。
  * 2バイトではなく、1バイトである点が、REPEATビットが0の時との違い。

## 外部メモリへのADPCMデータ書き込み時の動作について

外部メモリにADPCMデータをCPUから書き込むときの挙動の調査結果です。

### 通常の書き込み時の動作

32バイトのデータを外部メモリ書き込んだ時の挙動。
...
![通常の書き込み時の動作](../chart/mem_write.png)

* 1バイト目のレジスタ書き込みからMDEN信号はアサートされる。そのままメモリに書き込まれていると思われる。
  * ダミーリードのような動作はない。
* 32バイト目のデータを書き込んだ後、ステータスレジスタのEOSビットが1になる。

### STOP ADDRESSを超えて書き込んだ時の動作

32バイトのデータを外部メモリに書き込み、さらに続けて超えて書き込んだ時の挙動。
...
![STOP ADDRESSを超えて書き込んだ時の動作](../chart/mem_write_cont.png)

* 32バイト目のデータを書き込んだ後、ステータスレジスタのEOSビットが1になる。
* 33バイト目の書き込みでSTART ADDRESSに戻る。
